...
def must_be_in_class(view):
    @functools.wraps(view)
    def wrapped_view(**kwargs):
        if kwargs['chain_id'] not in g.accessible_chains:
            return redirect(url_for('auth.login'))
        return view(**kwargs)
    return wrapped_view


def must_be_lecturer(view):
    @functools.wraps(view)
    def wrapped_view(**kwargs):
        if not g.user['lecturer']:
            return redirect(url_for('auth.login'))
        return view(**kwargs)
    return wrapped_view
...
def load_logged_in_user():
    ...
    if user_id is None:
        ...
        g.accessible_chains = list(c['chain_id'] for c in get_db().execute(
            'SELECT chain_id FROM user_chain WHERE user_id = ?', (user_id,)
        ).fetchall())


@bp.route('/register-lecturer', methods=('GET', 'POST'))
